...
for tar in target_columns:
    ...
    # Hitung persentase nilai NaN dalam kolom target
    nan_percentage = (filtered_data[tar].isna().sum() / len(filtered_data)) * 100
    nan_percentage_results[tar] = nan_percentage

    # Simpan distribusi asli
...
# Membuat DataFrame untuk hasil persentase NaN
nan_percentage_df = pd.DataFrame.from_dict(nan_percentage_results, orient='index', columns=['NaNPercentage'])
nan_percentage_df = nan_percentage_df.reset_index().rename(columns={'index': 'Target'})
nan_percentage_df['NaNPercentage'] = nan_percentage_df['NaNPercentage'].map('{:.2f}%'.format)
average_nan = nan_percentage_df['NaNPercentage'].str.rstrip('%').astype(float).mean()
print("\nPersentase Nilai Noise(NaN) dari Semua Kolom Target:")
print(nan_percentage_df)

print(f"\nNilai Rata-Rata Noise(NaN) dari Semua Kolom Target: {average_nan:.2f}%")
